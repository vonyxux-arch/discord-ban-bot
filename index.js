...
// 🎛️ المكتبة الصوتية (Media Library) - يمكنك تعديل الأسماء من هنا
const MEDIA_LIBRARY = [
    {
        label: "تغطية مباشرة | قناة الجزيرة",
        description: "البث الحي والمباشر للأخبار 24/7",
        value: "https://www.youtube.com/live/bNyUyrR0PHo",
        emoji: "📡"
    },
    {
        label: "القضاء و القدر : بين الإيمان و العمل",
        description: "بودكاست محفوف - د. عبدالرحمن الحرمي",
        value: "https://youtu.be/am6jfmu7E_M",
        emoji: "🎙️"
    },
    {
        label: "How To Build Discipline",
        description: "Islamic perspective on self-discipline (Quran)",
        value: "https://youtu.be/9_g1UpYhFGo",
        emoji: "🧠"
    },
    {
        label: "تعفن دماغك ؟ تحس بوقتك؟",
        description: "بودكاست عبدالله العلاوي - تطوير الذات",
        value: "https://youtu.be/AiLGiRkTvSk",
        emoji: "🎧"
    },
    {
        label: "Live Stream / Broadcast (EN)",
        description: "Continuous Live Stream Media",
        value: "https://www.youtube.com/live/2lJZPT6OljI",
        emoji: "📻"
    },
    {
        label: "بث مباشر مستمر (عربي)",
        description: "تغطية وبث حي متواصل",
        value: "https://www.youtube.com/live/rojlkPMBgpY",
        emoji: "🎵"
    }
];

// المتغير الذي يحمل البث النشط حالياً (الافتراضي هو الأول في القائمة)
let CURRENT_STREAM = MEDIA_LIBRARY[0].value;
...
new SlashCommandBuilder()
.setName("media-hub")
.setDescription("Deploy the Audio & Media Control Panel (Admin Only)."),
...
async function triggerStreamPlayback() {
    ...
    try {
        const stream = await play.stream(CURRENT_STREAM).catch(err => {
...
// ---------------------------------------------------------
// نظام لوحة التحكم بالوسائط والصوت (Media Hub)
// ---------------------------------------------------------
if (interaction.isChatInputCommand() && interaction.commandName === "media-hub") {
    if (!interaction.member.permissions.has("Administrator")) {
        return interaction.reply({ content: "❌ لا تمتلك الصلاحيات الكافية لتنفيذ هذا الأمر.", ephemeral: true });
    }

    const mediaHubEmbed = new EmbedBuilder()
        .setColor("#1DB954") // لون أخضر فخم مستوحى من سبوتيفاي
        .setAuthor({ name: "YONKO MEDIA HUB", iconURL: interaction.guild.iconURL() })
        .setTitle("🎛️ وحدة التحكم المركزية بالبث الصوتي")
        .setDescription("مرحباً بك في لوحة تحكم الوسائط (Premium Hub).\nالرجاء اختيار المقطع، البودكاست، أو البث المباشر الذي تود تشغيله داخل الغرفة الصوتية من القائمة المنسدلة أدناه.\n\n`يتم تبديل المسار الصوتي بشكل فوري وتلقائي بمجرد الاختيار.`")
        ...
        .setTimestamp()
        .setFooter({ text: "YONKO TEAM • Audio Control Panel", iconURL: interaction.guild.iconURL() });

    const selectMenu = new StringSelectMenuBuilder()
        .setCustomId("media_selector")
        .setPlaceholder("🎧 اضغط هنا لاختيار البث أو المقطع الصوتي...")
        .addOptions(
    ...
    await interaction.reply({ content: "💻 تم تجهيز لوحة التحكم بالوسائط بنجاح.", ephemeral: true });
    return interaction.channel.send({ embeds: [mediaHubEmbed], components: [row] });
}

if (interaction.isStringSelectMenu() && interaction.customId === "media_selector") {
    if (!interaction.member.permissions.has("Administrator")) {
        return interaction.reply({ content: "❌ عذراً، لا تمتلك الصلاحيات لإدارة البث والتعديل عليه.", ephemeral: true });
    }
    
    const selectedUrl = interaction.values[0];
    CURRENT_STREAM = selectedUrl;
    
    const channel = interaction.guild.channels.cache.get(VOICE_CHANNEL_ID);
    ...
    const selectedMedia = MEDIA_LIBRARY.find(m => m.value === selectedUrl);
    
    const successEmbed = new EmbedBuilder()
        .setColor("#1DB954")
        .setTitle("▶️ تم تحديث البث بنجاح")
        .setDescription(`تم تغيير المسار الصوتي وإعادة التوجيه إلى:\n**${selectedMedia.label}**\n\n\`${selectedMedia.description}\``)
        .setTimestamp();
...
// ---------------------------------------------------------
// نظام البلاغات والحماية (Report System)
// ---------------------------------------------------------
